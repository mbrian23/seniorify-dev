...
const TEAMS_HOST = "teams.seniorify.dev";
...
export function middleware(request: NextRequest) {
  ...
  if (host === TEAMS_HOST) {
    ...
    // teams.seniorify.dev/ and /teams -> serve the for-teams landing
    if (path === "/" || path === "/teams") {
      url.pathname = "/teams";
      return NextResponse.rewrite(url);
    }

    // Anything else on the teams subdomain -> bounce to the primary domain
    const target = new URL(path + url.search, `https://${PRIMARY_HOST}`);
    return NextResponse.redirect(target, 308);
  }
